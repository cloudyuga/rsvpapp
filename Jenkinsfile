    ...
    stage('Build') {
      ...
      steps {
        container('docker') {
          ...
          // Publish new image
          sh "docker login --username $DOCKERHUB_CREDS_USR --password $DOCKERHUB_CREDS_PSW && docker push $DOCKERHUB_REPO:${env.GIT_COMMIT}"
        }
      }
    }
      ...
      steps {
        container('tools') {
          ...
          dir("kustomize-demo") {
              ...
              sh "cd ./overlays/staging && kustomize edit set image ${env.DOCKERHUB_REPO}:${env.GIT_COMMIT}"
